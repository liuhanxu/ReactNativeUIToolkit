...
import {StyleSheet, Text as RNText, View, Image} from 'react-native';
...
export default class ListItem extends Component {
  ...
  static propTypes = {
    ...
    detailStyle: RNText.propTypes.style,
  ...
  buildProps() {
    ...
    //style
    style = [{
      backgroundColor: Default.rowColor,
      paddingLeft: Default.rowPaddingLeft,
      paddingRight: Default.rowPaddingRight,
      paddingTop: Default.rowPaddingTop,
      paddingBottom: Default.rowPaddingBottom,
      minHeight: Default.rowMinHeight,
      overflow: 'hidden',
      flexDirection: 'row',
      alignItems: 'center',
    }].concat(style);
    ...
    if (typeof title === 'string' || typeof title === 'number') {
      ...
      title = <Text style={[textStyle, titleStyle]} type='title' text={title} />
    }

    //detail
    if (typeof detail === 'string' || typeof detail === 'number') {
      let textStyle = titlePlace === 'top' ? {lineHeight: Default.rowDetailLineHeight, color: Default.labelTextColor} : {flexGrow: 1, flexShrink: 1, textAlign: 'right'};
      if (title) {
        if (titlePlace === 'left') textStyle.paddingLeft = Default.rowPaddingTitleDetail;
        else textStyle.paddingTop = Default.rowPaddingTitleDetail;
      }
      if (!detailMultiLine && detailMultiLine !== false) {
        detailMultiLine = titlePlace === 'top';
      }
      detail = <Text style={[textStyle, detailStyle]} type='detail' text={detail} numberOfLines={detailMultiLine ? 0 : 1} />
    }

    //icon
    if ((icon || icon === 0) && !React.isValidElement(icon)) {
      icon = (
        <View style={{paddingRight: Default.rowIconPaddingRight}}>
          <Image style={{width: Default.rowIconWidth, height: Default.rowIconHeight}} source={icon} />
        </View>
      );
    }
    ...
    if (accessory && !React.isValidElement(accessory)) {
      ...
      switch (accessory) {
        case 'empty':
          ...
          break;
        case 'check':
          imageSource = require('../Checkbox/icon/check.png');
          tintColor = Default.rowAccessoryCheckColor;
          break;
        case 'indicator':
          imageSource = require('./icons/indicator.png');
          tintColor = Default.rowAccessoryIndicatorColor;
          break;
        default: imageSource = accessory;
      }
      let imageStyle = {
        width: Default.rowAccessoryWidth,
        height: Default.rowAccessoryHeight,
        tintColor: tintColor,
      };
      accessory = (
        <View style={{paddingLeft: Default.rowAccessoryPaddingLeft}}>
    ...
    let separatorStyle = {
      backgroundColor: Default.rowSeparatorColor,
      height: Default.rowSeparatorLineWidth,
    };
    let indentViewStyle = {
