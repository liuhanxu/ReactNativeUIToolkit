...
export default class ToastView extends Overlay.View {
  ...
  buildProps() {
    ...
    style = [{
      paddingLeft: Default.toastScreenPaddingLeft,
      paddingRight: Default.toastScreenPaddingRight,
      paddingTop: Default.toastScreenPaddingTop,
      paddingBottom: Default.toastScreenPaddingBottom,
      justifyContent: position === 'top' ? 'flex-start' : (position === 'bottom' ? 'flex-end' : 'center'),
      alignItems: 'center',
    }].concat(style);

    contentStyle = {
      backgroundColor: Default.toastColor,
      paddingLeft: Default.toastPaddingLeft,
      paddingRight: Default.toastPaddingRight,
      paddingTop: Default.toastPaddingTop,
      paddingBottom: Default.toastPaddingBottom,
      borderRadius: Default.toastBorderRadius,
      alignItems: 'center',
    };

    if (typeof text === 'string' || typeof text === 'number') {
      text = (
        <Text style={{color: Default.toastTextColor, fontSize: Default.toastFontSize}}>{text}</Text>
      );
    }
    ...
    if (icon || icon === 0) {
      ...
      if (!React.isValidElement(icon)) {
        ...
        image = (
          <Image
            style={{width: Default.toastIconWidth, height: Default.toastIconHeight, tintColor: Default.toastIconTintColor}}
            source={imageSource}
            />
        );
      } else {
      ...
      icon = (
        <View style={{paddingTop: Default.toastIconPaddingTop, paddingBottom: Default.toastIconPaddingBottom}}>
          {image}
        </View>
      );
    }
